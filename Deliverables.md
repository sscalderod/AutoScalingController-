...
└── DELIVERABLES.md          # este documento (diseño, ejecución, evidencia, análisis)
```
...
```bash
# Conectarse a la instancia de control vía Session Manager
...
# Dejarlo corriendo en background para que sobreviva a cerrar la sesión
nohup python3 autoscaler.py > controller_output.log 2>&1 &
disown
 
# Seguir las decisiones en vivo
tail -f /var/log/autoscaler/decisions.log
```
 
Antes de correr, verificar la infraestructura: ASG con al menos una
instancia `InService`, Target Group con el target en estado `healthy`, y
el DNS name del ALB respondiendo `OK <hostname>` en el navegador.
 
### 2.4 Generación de carga (load-generation)
 
Dos mecanismos:
 
1. **Manual/orgánica**: conectarse por Session Manager a una instancia
   `InService` del ASG (no la de control) y ejecutar:
```bash
   sudo dnf install -y stress-ng
   stress-ng --cpu 0 --timeout 300s
```
 
   Simula carga real de usuarios sin generar tráfico externo hacia el ALB
   (evita cualquier riesgo de que AWS interprete el tráfico como abuso).
 
2. **Automática (fallback del controller)**: si tras
   `IDLE_CYCLES_BEFORE_STRESS` ciclos no hay carga orgánica, el propio
   controller dispara `stress-ng` vía SSM Run Command — este es el
   mecanismo pedido explícitamente por el profesor en la especificación de
   la actividad de clase, y no requiere ninguna acción manual.
